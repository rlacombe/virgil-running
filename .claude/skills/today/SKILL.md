...
2. Fetch in parallel (call MCP tools directly — no subagents needed):
   ...
   - `get_weather` using the athlete's lat/lon from their profile — current conditions and forecast
   - Zones are cached in `athlete/profile.md` — skip `get_athlete` unless the zones section is missing
3. Display:
